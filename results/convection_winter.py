...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 30 14:04:04 2023

@author: gaj002
"""

"""This script creates potential plots with overlaid boundaries for both signs of IMF B_y during local winter"""
...
with open(f'data/lmfit_tilt_{tilt_titl}_{By_titl1}_cvar_{c_var_lim}_{filnavn}.pickle', 'rb') as handle:
    file_1_info = pickle.load(handle)
with open(f'data/lmfit_tilt_{tilt_titl}_{By_titl2}_cvar_{c_var_lim}_{filnavn}.pickle', 'rb') as handle:
    file_2_info = pickle.load(handle)


file_1 = vaex.open(f'data/lmfit_tilt_{tilt_titl}_{By_titl1}_cvar_0.04_{filnavn}.hdf5')
file_2 = vaex.open(f'data/lmfit_tilt_{tilt_titl}_{By_titl2}_cvar_0.04_{filnavn}.hdf5')
...
#Boundary estimation
bi1 = detect_boundaries_w_m(imgs1, HiLDA=True)
bi2 = detect_boundaries_w_m(imgs2, HiLDA=False)
